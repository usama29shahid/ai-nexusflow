...
with DAG(
    ...
    silver = BashOperator(
        task_id="silver",
        bash_command=elt_dbt_layer("tag:products,tag:staging"),
    )
    gold = BashOperator(
        task_id="gold",
        bash_command=elt_dbt_layer("tag:products,tag:gold"),
    )
    observability = BashOperator(
        task_id="observability",
        bash_command=elt_bash_command("./scripts/observability-publish-run.sh"),
    )
