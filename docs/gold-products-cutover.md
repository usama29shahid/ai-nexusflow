...
- Delete = **Pattern A expire** active row (`is_active=0`, `is_deleted=1` as **Int8**, `valid_to=scd_bound_at`); no new tombstone version row.
...
- [x] `uv run dbt test --select "test_type:unit"` (silver + gold; gold covers initial + truly-new / delete / change / reappear)
